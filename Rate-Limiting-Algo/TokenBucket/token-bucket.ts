class TokenBucket {
  ...
  private bucketMaxCapacity: number; // Maximum number of token a bucket can hold.
  private bucketFillRate: number; // Rate at which tokens are  added to the bucket (tokens/second).
  private tokenCount: number; // Current token count
  private lastTime: number; // Last time we checked the token count in milliseconds
